...
class ProjectData:
  ...
  def __init__(self, config, collator=None):
    if config.dataset_type == 'cifar10':
      datasets.CIFAR10(root=config.data_root,
                       download=False,
                       transform=T.Compose([
      ...
      self.data = datasets.CelebA(root=config.data_root,
                                  download=False,
                                  transform=T.Compose([
                                      T.CenterCrop(178),
                                      T.Resize((128, 128)),
      ...
      base_ds = Flickr30kDataset(images_root=f"{config.data_root}/flickr30k/Images",
      ...
      train_ds = Subset(
          Flickr30kDataset(f"{config.data_root}/flickr30k/Images",
                           f"{config.data_root}/flickr30k/captions.txt",
                           transform=train_tf), train_inds)
      val_ds = Subset(
          Flickr30kDataset(f"{config.data_root}/flickr30k/Images",
                           f"{config.data_root}/flickr30k/captions.txt",
                           transform=val_tf), val_inds)
...
class PrecomputedLatents(Dataset):
  ...
  def __getitem__(self, idx):
    ...
    assert x_img_1.dim() == 3 and x_img_1.size(
        0) == 4, f"bad img latent shape {tuple(x_img_1.shape)}"
