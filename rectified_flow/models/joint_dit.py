...
class TinyDiTLatent(nn.Module):
  ...
  def build_2d_sincos(self, d, h, w):
    ...
    coords = torch.stack([y, x], -1)  # [H,W,2]
    freqs = torch.exp(torch.linspace(0, math.log(10000), d // 4))

    def pe(a):
      return torch.stack([torch.sin(a[..., None] / freqs),
                          torch.cos(a[..., None] / freqs)], -1).flatten(-2)
  ...
  def forward(self, x_img_t, t, pooled_txt_t, tok_embeds=None, tok_mask=None):
    B = x_img_t.size(0)
    # image tokens + pos
    x = self.img_in(x_img_t).permute(0, 2, 3, 1).reshape(B, self.h8 * self.w8,
                                                         self.d) + self.pos.to(x_img_t.device)
    # time cond
