...
class FiLMAdapter(nn.Module):
  ...
  def __init__(self, txt_dim, target_dim):
    ...
    nn.init.xavier_uniform_(self.proj.weight)  # default-ish
...
class CrossAttention(nn.Module):
  ...
  def __init__(self, img_dim, txt_dim, n_heads=8):
    ...
    nn.init.xavier_uniform_(self.proj_out.weight)  # no tiny gain
    nn.init.zeros_(self.proj_out.bias)
    self.ca_gate = nn.Parameter(torch.tensor(1.0))
...
class UNetPixelPlusTCPool(nn.Module):
  ...
  def __init__(self, in_ch=3, time_dim=128, p_dropout=None):
    ...
    self.null_txt = nn.Parameter(torch.zeros(langvae_dim))
    nn.init.normal_(self.null_txt, std=0.02)


  def forward(self, x_img_t, x_txt_pool_t, t, is_uncond: torch.Tensor=None):
    if is_uncond is not None:
      # replace selected rows with the learnable null embedding
      x_txt_pool_t = x_txt_pool_t.clone()
      x_txt_pool_t[is_uncond.bool()] = self.null_txt
...
class UNetPixelPlusTCTxtTok(nn.Module):
  ...
  def __init__(self, in_ch=3, time_dim=128, p_dropout=None):
    ...
    img_dim = 1024
    clip_dim = 512
    self.cross_attn = CrossAttention(img_dim=img_dim, txt_dim=clip_dim, n_heads=4)
    self.film = FiLMAdapter(txt_dim=clip_dim, target_dim=img_dim)

    self.image_out = nn.Conv2d(128, in_ch, 1)

    # Learnable unconditional (null) embedding for CFG
    self.null_token = nn.Parameter(torch.zeros(1, 1, clip_dim))
  ...
  def forward(self, x_img_t, txt_toks, t, attn_mask: torch.Tensor=None, is_uncond: torch.Tensor=None):
    """
    txt_toks.shape (B, L, CL)
    """

    if is_uncond is not None:
      m = is_uncond.bool().view(-1, 1, 1)   # (B,1,1)
      B, L, C = txt_toks.shape
      null_full = self.null_token.expand(B, L, C)
      txt_toks = torch.where(m, null_full, txt_toks)
    ...
    ##### FiLM #####
    _, C_m, H_m, W_m = m.shape
    pooled_text = self._pool_text_tokens(txt_toks, attn_mask)
    γ, β = self.film(pooled_text)                               # (, 1024), (, 1024)
    γ = γ[:, :, None, None]                                     # (, 1024, 1, 1)
    β = β[:, :, None, None]                                     # (, 1024, 1, 1)
    m = m * (1 + γ) + β                                         # (, 1024, 4, 4)
    ...
    ##### Cross Attention #####
    img_toks = m.flatten(2).permute(0, 2, 1)                    # (, 4*4, 1024)
    m = self.cross_attn(img_toks, txt_toks, attn_mask)          # (, 4*4, 102)
    m = m.permute(0, 2, 1).view(B, C_m, H_m, W_m)
...
class UNetPixelPlusTCTxtTokRedChan(nn.Module):
  ...
  def __init__(self, in_ch=3, time_dim=128, p_dropout=None):
    ...
    # Down path (keep blocks shape-preserving; downsample between levels)
    self.enc1 = UnetLayer(in_ch, 64, time_dim, p_dropout)
    self.ds1 = Downsample(64)

    self.enc2 = UnetLayer(64, 128, time_dim, p_dropout)
    self.ds2 = Downsample(128)
    self.enc_sa2 = AttnBlock(c=128, heads=8)

    self.enc3 = UnetLayer(128, 256, time_dim, p_dropout)
    self.ds3 = Downsample(256)
    self.enc_sa3 = AttnBlock(c=256, heads=8)

    self.enc4 = UnetLayer(256, 512, time_dim, p_dropout)
    self.ds4 = Downsample(512)

    self.mid = ResnetBlock(512, 512, time_dim, p_dropout)

    # Up path (upsample + concat + block)
    self.us4 = Upsample(512)
    self.dec4 = UnetLayer(512 + 512, 512, time_dim, p_dropout)
    self.dec_sa4 = AttnBlock(c=512, heads=8)

    self.us3 = Upsample(512)
    self.dec3 = UnetLayer(512 + 256, 256, time_dim, p_dropout)
    self.dec_sa3 = AttnBlock(c=256, heads=8)

    self.us2 = Upsample(256)
    self.dec2 = UnetLayer(256 + 128, 128, time_dim, p_dropout)

    self.us1 = Upsample(128)
    self.dec1 = UnetLayer(128 + 64, 64, time_dim, p_dropout)
    ...
    self.image_out = nn.Conv2d(64, in_ch, 1)
