...
def train_test_model(config):
  ...
  tok_path = "openai/clip-vit-base-patch32"
  ...
  ###### Model / EMA / Optimizer ######
  model = UNetPixelPlusTCTxtTokRedChan(in_ch=config.num_channels, time_dim=config.time_dim,
  ...
  for epoch in range(1, config.n_epochs+1):
    ...
    with tqdm(train_dl, desc="Training") as pbar:
      ...
      for images, token_ids, attn_mask in pbar:
        ...
        v, v_pred = tu.compute_data_tc_clip_cfg(model, text_enc, images,
                                                token_ids, attn_mask,
    ...
    ################################################
    for images_v, token_ids_v, attn_mask_v in val_dl:
      if images_v.size(0) >= 4:
        break # use this batch
    images_v    = images_v.to(DEVICE, non_blocking=True)
    token_ids_v = token_ids_v.to(DEVICE, non_blocking=True)
    attn_mask_v = attn_mask_v.to(DEVICE, non_blocking=True)

    trip = tu.text_triplet_metrics(model, text_enc, images_v, token_ids_v, attn_mask_v, DEVICE)
    wandb.log({
        "text/mse_c":  trip["mse_c"],
        "text/mse_u":  trip["mse_u"],
        "text/mse_w":  trip["mse_w"],
        "text/delta_u": trip["delta_u"],
        "text/delta_w": trip["delta_w"],
        "epoch": epoch,
    })

    txt_toks = text_enc(input_ids=token_ids_v, attention_mask=attn_mask_v).last_hidden_state.to(DEVICE) # (1, L, TH)
    stats = tu.null_proximity_stats(model, txt_toks, attn_mask_v)
    wandb.log({
        "text/null_cos":   stats["null_cos"],      # high = collapse-y
        "text/null_l2":    stats["null_l2"],       # scale-dependent
        "text/null_ratio": stats["null_ratio"],    # scale-free, best to watch
        "epoch_t": epoch + 0.99999,
    })
    ...
    if (epoch % config.inference_peek_num) == 0:
      ...
      if config.save_model:
        ...
        gdio.save_and_upload_model(service, model, config,
                                   drive_path=f"rf_ckpts/{config.name}",  # auto-creates nested folders
...
def load_and_test_model(config):
  model = UNetPixelPlusTCTxtTok(in_ch=config.num_channels, time_dim=config.time_dim,
                        p_dropout=config.p_dropout).to(DEVICE)
  model = tu.load_model_from_local(model, "unet-pp-tc-pixel-img32-flickr30k-cfg")
  ...
  gains_c_vs_u, gains_c_vs_w = tu.text_effect_eval(model, text_enc, val_dl, DEVICE)
  print(gains_c_vs_u)
  print(gains_c_vs_w)
...
def main():
  ...
  print(f"env={env}")
  config = tu.load_config_01(path="config/train_unet_pp_pixel_flickr_tc_clip", env=env)
