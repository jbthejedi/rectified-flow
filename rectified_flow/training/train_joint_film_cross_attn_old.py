...
def train_test_model(config):
  ...
  IH = 4
  model = JointFiLMCrossAttn(img_shape=IMAGE_SHAPE, img_dim=4, txt_dim=langvae_proj_dim,
  ...
  for epoch in range(1, config.n_epochs + 1):
    ...
    with tqdm(train_dl, desc="Training") as pbar:
      ...
      for (images, token_ids, attn_mask), t_fetch in timed_iter(pbar):
        v_pred, u_pred, v_star_img, u_star_txt = compute_data(images, token_ids, attn_mask, aekl,
                                                              langvae, model, device)
    ...
    with torch.no_grad():
      model.eval()
      imgs, sentences = sample_joint_batch_vae(model, aekl, langvae,
                                               batch_size=4, num_steps=50,
                                               img_shape=(IH, *IMAGE_SHAPE))
